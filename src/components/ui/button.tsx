...
const iconAnimations = {
  none: "",
  slide: "group-hover:translate-x-0.5 transition-transform",
}
...
export interface ButtonProps
  ...
  iconAnimation?: "none" | "slide"
}

const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { className, variant, size, asChild = false, startIcon, endIcon, iconAnimation, children, ...props },
    ref
  ) => {
    ...
    return (
      ...
      >
        {children}
        {startIcon && startIcon}
        {endIcon && <span className={cn(iconAnimation && iconAnimations[iconAnimation])}>{endIcon}</span>}
      </Comp>
    )
  }
)
